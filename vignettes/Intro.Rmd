...
home.environments <- c(.08, .13, .18, .23)
```
...
```{r}
curriculum.start.points <- matrix(c(.12, .20), nrow=2, ncol=1)
```
...
The curriculum width describes the *span* of the curriculum. The full itensity of the school curriculum is located at the starting point plus the width. So the Kindergarten curriculum here begins at the achievement level of $0.12$ and ends at $0.12 + .08 = 0.20$. The first grade curriculum begins at $.20$ and ends at $.28$. 

```{r}
curriculum.widths <- matrix(c(.08, .08), nrow=2, ncol=1)
```
...
The ```curriculum.lower.slope=``` and ```curriculum.upper.slope=``` arguments describe how rapidly the curricular intensity fades for achievement levels below the lower bound of full intensity (e.g., review) and above the upper bound (e.g., advanced content). In general, values between 1-10 decribe slow fadeout while values in the hundreds specify quick fadeout.
...
**The best way to develop an intuition about these values is the experiment with the code for plotting the school curriculum function.**
...
The home curriculum describes learning outside of school. Based on a beta distribution, the default values for the ```home.curriculum.shape1=``` and ```home.curriculum.shape2=``` arguments (which are the ```shape1``` and ```shape2``` argument to ```dbeta()```) desribe the following shape to the home curriculum.
...
x <- seq(0, 1, by=.01)
y <- dbeta(x, shape1=home.curriculum.shape1, shape2=home.curriculum.shape2) 
...
Three students will be created that are identical in every way except for their initial achievement. 
...
trajectories <- ZPDGrowthTrajectories(output.format="wide", days=200, assignment=assignment,
                                      ...
                                      zpd.scale=.15, decay.weight=.005, useGPU=FALSE, verbose=TRUE)
...
It is also possible to fit statistcal models to the synthetic data. This is useful for atttempting to reproduce empirical results from synthetic data. The process is as follows:
